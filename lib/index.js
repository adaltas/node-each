...
Each = function(_elements, options = {}) {
  ...
  this._endable = 1;
  this._listeners = [];
  this._queuing = this.options.queue;
  // Public state
...
Each.prototype._handle_close = function(error, count) {
...
Each.prototype._run = function() {
  ...
  // This is the end
  error = null;
  if (!this._queuing && this._endable === 1 && (this._close || (handlers && this.done === this.total * this.options.times * handlers.length) || (this._errors.length && this.started === this.done))) {
    // if not @_queuing
    this._listeners.shift();
    ...
    if (this._errors.length || !this._has_next_handler()) {
      ...
      this._handle_close(error, this.done);
      return;
    }
    handlers = this._get_current_handler();
    this._endable = 1;
...
Each.prototype.write = Each.prototype.push = function(item) {
  var elements_l, l, ref;
  l = arguments.length;
  elements_l = l === 1 ? this._elements.length : ((ref = this._keys) != null ? ref.length : void 0) || 0;
  if (l === 1) {
