...
Each.prototype._get_next_handler = function() {
...
Each.prototype._call_next_then = function(error, count) {
  var i, j, len, listener, occurences, ref, ref1;
  occurences = 0;
  ref = this.listeners;
  for (i = j = 0, len = ref.length; j < len; i = ++j) {
    ...
    if (listener[0] === 'then' && occurences === this._handler_index) {
      if (((ref1 = this.listeners[i - 1]) != null ? ref1[0] : void 0) === 'error') {
        if (!error) {
...
Each.prototype._run = function() {
  var args, err, error, error1, handler, handlers, index, j, l, len, self;
  if (this.paused) {
    return;
  }
  error = null;
  ...
  while ((this.options.concurrency === true ? (this.total * this.options.times - this.started) > 0 : Math.min(this.options.concurrency - this.started + this.done, this.total * this.options.times - this.started))) {
    ...
    try {
      self = this;
      handlers = this._get_next_handler();
